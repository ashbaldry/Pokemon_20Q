...
# pokeref - Data.table of the remaining Pok̩mon left to guess from
...
PokeQuestion <- function(pokeguess, pokeref, pokeeggs, poketypes) {
  ...
  if(qtype == 'Weight') {
    ...
    pokeQ <- list(Question = paste0('Is the Pok̩mon ', direc, ' than ', median(pokeref$weight), 'kg?'),
  ...
  if(qtype == 'Height') {
    ...
    pokeQ <- list(Question = paste0('Is the Pok̩mon ', direc, ' than ', median(pokeref$height), 'm?'),
                  Smaller = direc == 'smaller')
  }

  ####Mega####
  if(qtype == 'Mega') {
    pokeQ <- list(Question = 'Can the Pok̩mon mega evolve?')
  }
  ...
  if(qtype == 'Generation') {
    ...
    if(length(minvers) == 1) {
      pokeQ <- list(Question = paste0('Was the Pok̩mon\'s first occurance in version ', minvers, '?'), Version = minvers)
    } else {
      pokeQ <- list(Question = paste0('Was the Pok̩mon\'s first occurance in any of the following versions: ',
  ...
  if(qtype == 'Colour') {
    ...
    pokeQ <- list(Question = paste0('Is the Pok̩mon primarily ', tolower(col), '?'), Colour = col)
  }

  ####Shape####
  if(qtype == 'Shape') {
    shape <- sample(pokeref$shape, 1)
    pokeQ <- list(Question = paste0('Is the Pok̩mon a ', shape, '?'), Shape = shape)
  }

  ####Baby####
  if(qtype == 'Baby') {
    pokeQ <- list(Question = 'Is the Pok̩mon a baby Pok̩mon?')
  }

  ####EvoStage####
  if(qtype == 'EvoStage') {
    evostage <- sample(pokeref$evo_stage, 1)
    pokeQ <- list(Question = paste('Is the Pok̩mon in it\'s',
  ...
  ####HasEvolution####
  if(qtype == 'HasEvolution') {
    pokeQ <- list(Question = 'Does the Pok̩mon have an evolution (not including mega evolution)?')
  }

  ####HasEvolved####
  if(qtype == 'HasEvolved') {
    pokeQ <- list(Question = 'Does the Pok̩mon evolve from another Pok̩mon (not including mega evolution)?')
  }
  ...
  if(qtype == 'Type') {
    ...
    pokeQ <- list(Question = paste0('Is it a', if(substr(type, 1, 1) %in% c('A', 'E', 'I', 'O', 'U')) 'n',
                                    ' ', tolower(type), ' type Pok̩mon?'), Type = type)
  }

  ###DualType###
  if(qtype == 'DualType') {
    pokeQ <- list(Question = 'Does the Pok̩mon have two types?')
  }

  ####Egg####
  if(qtype == 'Egg') {
    eggtype <- sample(pokeeggs$egg_group_name, 1)
    pokeQ <- list(Question = paste('Is the Pok̩mon part of the', tolower(eggtype), 'egg family?'), EggType = eggtype)
  }
  ...
  if(qtype == 'TypeEff') {
    ...
    if(typeeff == 'No Effect') {
      pokeQ <- list(Question = paste0('Does a', if(substr(type, 1, 1) %in% c('A', 'E', 'I', 'O', 'U')) 'n', ' ',
                                      tolower(type), ' attack have ', tolower(typeeff), ' on the Pok̩mon?'),
                    Type = type, Effect = typeeff)
    } else {
      pokeQ <- list(Question = paste0('Is a', if(substr(type, 1, 1) %in% c('A', 'E', 'I', 'O', 'U')) 'n', ' ',
                                      tolower(type), ' attack ', tolower(typeeff), ' on the Pok̩mon?'),
                    Type = type, Effect = typeeff)
    }
  }
